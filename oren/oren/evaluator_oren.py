...
class OrenEvaluator(EvaluatorBase):
    """
    Evaluator for OREN models.
    """
    ...
    def forward_model(
        ...
        if len(sdf_prior) == 1:
            ...
            sdf_prior = sdf_prior[0].to(device)
            sdf_residual = sdf_residual[0].to(device)
            ...
            sdf_residual = torch.cat(sdf_residual, dim=0).to(device)
